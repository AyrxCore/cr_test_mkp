...
class UpplerOrderService extends HttpClientProvider
{
    /**
    ...
    public function getOrdersByUserId(int $userId): array|null
    {
        try {
            $res = $this->request(
                'GET',
                $this->apiUrl . 'v1/buyer/order/?criteria[user]=' . $userId . '&expand[]=buyer_user&criteria[type]=order',
            );
    ...
    public function getOrderByIdAndUserId(int $orderId, int $userId): \stdClass
    {
        $res = $this->request(
            'GET',
            $this->apiUrl . 'v1/buyer/order/' . $orderId . '?expand[]=items&expand[]=shipments',
        );
    ...
    public function getOrderInvoiceByIdAndUserId(int $orderInvoiceId): array
    {
        $res = $this->request(
            'GET',
            $this->apiUrl . 'v1/buyer/payment/' . $orderInvoiceId . '/download',
        );
